...
// TestTransactionOperationsMatch tests TransactionOperationsMatch predicate.
func TestTransactionOperationsMatch(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			require.Equal(t, tt.expected, TransactionOperationsMatch(tt.ops, tt.allowed))
		})
	}
}
